...
@property (nonatomic, strong, nullable) GMUserFileSystem *fs;
@property (nonatomic, strong, nullable) LoopbackFS *loop;
...
- (void)mountFailed:(NSNotification *)notification
{
    ...
    NSError *error = userInfo[kGMUserFileSystemErrorKey];
...
- (void)applicationDidFinishLaunching:(NSNotification *)notification
{
    ...
    NSArray<NSURL *> *paths = [panel URLs];
    if (paths.count != 1) {
        exit(0);
    }
    NSString *rootPath = nil;
    rootPath = paths[0].path;
    ...
    self.loop = [[LoopbackFS alloc] initWithRootPath:rootPath];
    self.fs = [[GMUserFileSystem alloc] initWithDelegate:self.loop isThreadSafe:NO];

    NSMutableArray *options = [NSMutableArray array];

    /*
     * Do not use the 'native_xattr' mount-time option unless the underlying file system supports
     * native extended attributes. Typically, the user would be mounting an HFS+ directory through
     * LoopbackFS, so we do want this option in that case.
     */
    [options addObject:@"native_xattr"];

    [options addObject:@"volname=LoopbackFS"];
    [self.fs mountAtPath:LoopbackMountPath
             withOptions:options];
}
